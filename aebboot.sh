...
#
# WHERE THIS LIVES. Pioneered in `aeo` so the whole loop (edit -> commit ->
# observe raw.githubusercontent redeploy -> run) is in one repo. Once stable it
# MOVES to the aeb repo root, and the curl URL above changes accordingly.
#
# ---------------------------------------------------------------------------
# AEBBOOT_REV: 6
